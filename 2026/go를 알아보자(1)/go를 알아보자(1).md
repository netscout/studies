...
- [mongodb야 반갑다!](#mongodb야-반갑다)
  - [go를 설치하자](#go를-설치하자)
  - [프로젝트 셋업](#프로젝트-셋업)
  - [그럼 코드를 작성해보자](#그럼-코드를-작성해보자)
  - [mongodb를 띄우자](#mongodb를-띄우자)
  - [이제 결과를 보자](#이제-결과를-보자)
  - [실습 환경 정리하기](#실습-환경-정리하기)
- [참고자료](#참고자료)
...
> 코드를 보면 Go의 특징적인 에러 처리 패턴을 볼 수 있습니다.
>
> ```go
> if err != nil {
> 	log.Fatal(err)
...
> ```
>
>다른 언어의 `try-catch` 와는 다르게, Go는 에러를 명시적으로 반환값으로 처리합니다. 이게 처음엔 좀 번거로워 보이지만, 에러가 발생할 수 있는 지점을 명확하게 알 수 있다는 장점이 있죠. 예외가 여러개 중첩 된 경우 예외가 어디서 발생한 건지 어렵게 찾을 필요가 없습니다!

> 그리고 코드를 실행할 때마다 같은 데이터가 계속 쌓이는 게 싫다면, 삽입 전에 기존 데이터를 삭제하는 코드를 추가할 수도 있습니다.
>
> ```go
> // 삽입 전에 기존 데이터 삭제 (선택사항)
> _, err = collection.DeleteMany(ctx, bson.M{"email": "john.doe@example.com"})
> if err != nil {
> 	log.Fatal(err)
> }
> ```
...
간단한 `docker-compose.yml` 을 사용해서 mongodb를 띄울 겁니다. 프로젝트 루트 폴더(mongodb-example)에 `docker-compose.yml` 파일을 생성해주세요.
...
실습이 끝났으면 깔끔하게 정리해줍시다. MongoDB 컨테이너를 종료하고 데이터까지 삭제하려면 다음 명령어를 실행하면 됩니다.

```bash
> docker-compose down -v
```

`-v` 옵션은 볼륨까지 삭제한다는 의미입니다. 실습용 데이터니까 깔끔하게 지워버리는 게 속이 시원하죠!
