...
1대1로 요리대결을 할 때는 동시성의 문제가 절대적입니다. 제한된 시간 내에 국을 끓이면서 재료를 손질하다가 오븐에 재료를 넣고 다시 국을 젓고 등의 작업을 어떻게 번갈아가면서 동시에 진행할지 잘 설계해야 되죠. 반면에 n대n으로 요리대결을 할 때는 동시성과 병렬성을 함께 고려해야 합니다. 각자의 작업 배분과 진행 순서를 잘 설계한 다음에 실수 없이 동시에 각자 할 일을 잘 진행해야만 하죠.
...
Go는 `동시성을 위해서 goroutine을 사용합니다.` goroutine은 OS 스레드와 달리 매우 작은 2KB의 스택에서 시작하기 때문에 수십만 개까지도 생성이 가능하고, OS 스레드에서 처리할 goroutine을 쉽게 전환할 수 있기 때문에 goroutine의 전환은 OS 스레드의 문맥 교환(context switching)이 필요하지 않습니다.
...
지금까지 확인한 내용을 정리해보면,
- CPU를 많이 사용하는 작업은 이벤트 루프의 콜백에서 처리될 경우, 이벤트 루프를 블록하게 됩니다. 그래서 `worker_threads`를 사용해서 별도의 스레드로 처리하거나, 작업을 아주 작게 쪼개서 이벤트 루프가 막히지 않도록 해야 합니다.
- `nextTick`, `promise`의 콜백 안에서 `nextTick`을 계속 걸면 이벤트 루프는 다음으로 진행하지 못합니다. 그래서 타이머나 I/O의 처리가 매우 지연될 수 있습니다.
- 마찬가지로 `promise` 콜백 안에서 계속 `promise`를 걸어도 이벤트 루프는 다음으로 진행하지 못합니다.
- `nextTick`은 `지금 콜백이 끝난 직후에 처리해야 하는 작업`에만 예외적으로 사용하며, 다음 루프에 처리하는 게 목적이라면 `setImmediate`를 사용합니다.
- `setTimeout(0)`보다 `setImmediate`를 사용합니다. `setTimeout`은 아무리 짧게 잡아도 최소 1ms의 지연이 필요합니다. 그리고 `setImmediate`는 I/O 콜백을 처리하는 `Poll` 단계 다음으로 오는 `Check` 단계에서 처리되므로, I/O 콜백에서 `setImmediate`를 걸어두면 항상 `setTimeout(0)` 보다 먼저 처리됩니다.
- `setInterval` 타이머는 정확하지 않습니다. 타이머가 지정된 시간에 도달하더라도 콜백을 처리하는 `Poll` 단계가 오래 걸리면 처리가 밀리기 때문입니다. 다음 처리 시간을 `setTimeout`으로 설정하는 게 좀 더 정확한 처리가 가능합니다.
